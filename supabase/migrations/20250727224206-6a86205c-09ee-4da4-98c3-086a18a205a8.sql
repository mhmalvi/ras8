...
-- Update functions with proper search_path to fix security warnings
...
BEGIN
  ...
  ai_stats AS (
    ...
    FROM public.ai_suggestions ai
    JOIN public.returns r ON r.id = ai.return_id
    WHERE r.merchant_id = p_merchant_id
  ),
  return_reasons AS (
    SELECT 
...
-- Update analytics function with proper search_path
